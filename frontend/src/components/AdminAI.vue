<template>
    <div class="chatbot">
      ...
      <textarea ref="chatInput" v-model="userMessage" placeholder="Enter a message..." required></textarea>
      <span id="send-btn" class="emoji-icon" @click="handleChat">➤</span>
    </div>
    </div>
  </template>
  ...
  export default {
    ...
    return {
      userMessage: "",
      API_KEY: "sk-k5d3ZrCZ5GYQKMK9kKliT3BlbkFJTixra70uBiFV3KeBZs",
      inputInitHeight: 0,
    };
    },
    mounted() {
    ...
    methods: {
      createChatLi(message, className) {
        ...
        let chatContent =
          className === "outgoing"
            ? `<p></p>`
            : `<span class="emoji-robot">🤖</span><p></p>`;
        ...
        fetch(API_URL, requestOptions)
          ...
          .catch((error) => {
            ...
            messageElement.textContent =
              "Oops! Sorry the counselor is offline. Please try again later.";
          })
          .finally(() =>
    ...
    this.$refs.chatInput.style.height = `${this.inputInitHeight}px`;
      ...
      handleEnterKey(e) {
        // if enter key is pressed without the shift and window key
